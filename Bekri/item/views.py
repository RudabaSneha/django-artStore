...
def delete(request,pk):
    item = get_object_or_404(Items,pk=pk, creator=request.user)
    item.delete()
...
def browse(request):
    browse = Items.objects.filter(is_sold=False)
    ...
    if query:
        browse = browse.filter(Q(name__icontains=query)| Q(description__contains=query))
    
    return render(request, 'item/browse.html', {'items': browse,'query':query, 'categories': categories, 'category_id':int(category_id)})